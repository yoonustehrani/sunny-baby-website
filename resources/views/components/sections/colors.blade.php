<section class="tw:flex tw:justify-center tw:mb-12">
    <div class="tw:bg-alabaster-dark tw:shadow-md tw:rounded-2xl tw:w-11/12 tw:lg:w-4/5 tw:py-8 tw:px-4">
        <div class="tw:w-fit tw:mx-auto">
            <div class="flat-title">
                <span class="title wow fadeInUp tw:font-bold" data-wow-delay="0s">دسته بندی بر اساس رنگ</span>
                <p class="sub-title wow fadeInUp" data-wow-delay="0s">طبق رنگ مورد سلیقه تان در میان صدها محصول، محصول
                    مدنظرتان را بیابید.</p>
            </div>
        </div>
        <div class="tw:-mt-6 tw:w-full tw:px-1 tw:md:px-4 tw:md:w-4/5 tw:mx-auto tw:flex-wrap tw:flex tw:items-center tw:justify-center tw:gap-8 tw:py-6">
            @foreach ($attribute->options as $option)
                <a wire:key="color-{{ $option->id }}" class="tw:block" href="{{ urldecode(route('pages.shop', ['filters' => [$attribute->id => [$option->id]] ])) }}">
                    <div
                        class="tw:size-20 tw:p-2 tw:rounded-full tw:bg-white tw:border tw:border-black/10 tw:relative tw:flex tw:justify-center">
                        <div class="tw:size-full tw:rounded-full" style="background: {{ makeGradient($option->content) }};"></div>
                        <span
                            class="tw:absolute tw:block tw:-bottom-1 tw:text-center tw:w-full tw:bg-white! tw:shadow-md tw:rounded-md tw:font-bold">{{ $option->label }}</span>
                    </div>
                </a>
            @endforeach
        </div>
    </div>
</section>