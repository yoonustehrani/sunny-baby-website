<tr @if($product->isVariant()) x-bind:class="{ 'hidden': displayVariable != {{ $product->parent_id }} }" @endif>
    ...
    <td class="px-5 py-4 sm:px-6">
        <span class="text-gray-200">{{ $product->affiliate_price ? format_price($product->affiliate_price) : '---' }}</span>
    </td>
    <td class="px-5 py-4 sm:px-6">
                        ...
                        <div
                            ...
                            <div class="tw:max-h-full tw:flex tw:items-center tw:relative tw:justify-center tw:w-8">
                                <div wire:loading.delay.remove >
                                    <p class="tw:bg-gray-100 tw:outline-0 tw:text-gray-800 tw:h-6 tw:w-full tw:text-center tw:p-0 tw:m-0 tw:border-0">{{ $count }}</p>
                                </div>
